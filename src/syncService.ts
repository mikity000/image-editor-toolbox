...
export interface SerializedBackgroundState {
  type: 'background';
  src: string;
  origImgW: number;
  origImgH: number;
...
export interface SerializedNormalImageState {
  type: 'image';
  src: string;
  left?: number;
  top?: number;
...
export type SerializedImageState = SerializedBackgroundState | SerializedNormalImageState;
...
export function serializeImages(
  canvas: Canvas | any,
  options: SerializeOptions = { includeBackground: true, includeImages: true }
): SerializedImageState[] {
  if (!canvas) return [];
  const { includeBackground = true, includeImages = true } = options;
  const states: SerializedImageState[] = [];
  ...
  if (includeImages) {
    ...
    objects
      .filter((o: any) => o instanceof FabricImage)
      .forEach((o: any) => {
        const matrix = o.calcTransformMatrix ? o.calcTransformMatrix() : [1, 0, 0, 1, 0, 0];
        const p1 = new Point(0, 0);
        const p2 = new Point(o.width ?? 0, o.height ?? 0);
        const t1 = p1.transform(matrix);
        const t2 = p2.transform(matrix);
...
export function restoreImages(
  canvas: Canvas | any,
  imageStates: SerializedImageState[] = [],
  options: SerializeOptions = { includeBackground: true, includeImages: true }
): Promise<void> {
  ...
  if (includeBackground) {
    ...
    if (idx !== -1) {
      const s = states.splice(idx, 1)[0] as SerializedBackgroundState;
      if (s.src) {
        promises.push(
          new Promise<void>(resolve => {
            ...
            imgEl.onload = () => {
              ...
              const bgImg: any = new FabricImage(imgEl, {
                left,
                top,
                scaleX: scale,
                scaleY: scale,
    ...
    states.forEach(state => {
      ...
      promises.push(
        new Promise<void>(resolve => {
          ...
          imgEl.src = state.src;
          imgEl.onload = () => {
            const inst: any = new FabricImage(imgEl, {
              left: state.left,
              top: state.top,
              scaleX: state.scaleX,
              scaleY: state.scaleY,
...
// ─────────────────────────────────────
// 大きいデータはチャンク化して送信
export function sendChunk(socket: Socket | any, payload: any, eventName: string): void {
  if (!socket) return;
...
export interface SetupSyncReturn {
  socket: Socket;
  emitSync: (opts?: SerializeOptions) => void;
  cleanup: () => void;
}

// ─────────────────────────────────────
// Socket.IO を使って同期ロジックを初期化
export function setupSync(canvas: Canvas | any, options: SetupSyncOptions = {}): SetupSyncReturn {
  ...
  const handleChunk = ({ id, index, total, chunk }: { id: string; index: number; total: number; chunk: string }) => {
    receiveChunk(incoming, onReceive, id, index, total, chunk);
  };

  socket.on('canvas:sync-chunk', handleChunk);

  const emit = (opts: SerializeOptions = { includeBackground: true, includeImages: true }) => {
...
export interface SetupListSyncOptions {
  url?: string;
  onReceive?: (data: any) => void;
}

export interface SetupListSyncReturn {
  socket: Socket;
  emitListSync: (listData: any) => void;
  cleanup: () => void;
}

// ─────────────────────────────────────
// 画像一覧同期専用セットアップ
export function setupListSync(options: SetupListSyncOptions = {}): SetupListSyncReturn {
  ...
  const emitListSync = (listData: any) => sendChunk(socket, listData, 'list:sync-chunk');
