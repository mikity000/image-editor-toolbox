 ...
 */
import { PAINT_CONFIG } from '../../../shared/constants/Constants';
 ...
 * @param hex '#RRGGBB' または '#RGB'
 * @param alpha 0.0 〜 1.0 (デフォルト 1.0)
 */
export function hexToRgba(hex: string, alpha: number = 1.0): RgbaColor {
 ...
 */
function getBgImageData(bgCanvas: HTMLCanvasElement | null | undefined, width: number, height: number): ImageData {
  if (bgCanvas) {
 ...
 * @param startX 開始X座標
 * @param startY 開始Y座標
 * @param isMatchFn 通過判定コールバック
 * @param initialVisited 初期訪問済みマップ (省略時は新規作成)
 * @returns 塗りつぶし領域マスク
 */
export function runScanlineFill(
  width: number,
  height: number,
  startX: number,
  startY: number,
  isMatchFn: (x: number, y: number) => boolean,
  initialVisited: Uint8Array | null = null
): Uint8Array {
 ...
 * @param width 幅
 * @param height 高さ
 * @param radius 閉処理半径 (gapClosing / 2)
 * @returns 隙間のみが塞がれたバリアマスク
 */
...
export function performFloodFill(
  ...
  expandRadius: number = PAINT_CONFIG.FLOOD_FILL_EXPAND_RADIUS,
  ...
  // ピクセル通過判定
  const isMatch = (px: number, py: number): boolean => {
    const pos = py * width + px;
    if (barrierMap && barrierMap[pos] === 1) return false;
  ...
  // 線の境界への自然な食い込み（expandRadius）のみを適用
...
export function performSmartObjectFill(
  ...
  const getSobelMagnitude = (px: number, py: number): number => {
 ...
 */
function applyMaskToPaintCanvas(paintCanvas: HTMLCanvasElement, mask: Uint8Array, fillRgba: RgbaColor): void {
  const width = paintCanvas.width;
  const height = paintCanvas.height;
