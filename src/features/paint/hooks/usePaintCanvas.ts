...
import { PAINT_CONFIG, APP_CONFIG, PDF_CONFIG, IMAGE_CONFIG } from '../../../shared/constants/Constants';
import { loadPdfDocument, renderPdfPage } from '../../pdf/utils/pdfRenderUtils';
import { fileToDataUrl } from '../../../shared/utils/imageUtils';
import { convertToWebP } from '../../../shared/utils/webpConverter';
import { performUnifiedFill } from '../utils/floodFillUtils';
import { PDFDocument } from 'pdf-lib';
import { saveAs } from 'file-saver';
import { Point, Size } from '../../../shared/types/common';
import {
  PaintMediaType,
  PaintToolType,
  FillMode,
  ...
  PageHistory,
  PageHistoryEntry,
  UsePaintCanvasReturn,
} from '../types/paint';
import { useToast } from '../../../shared/context/ToastContext';

// Shiftキー直線描画用 45度刻み（水平・垂直・45度斜め）の単位ベクトル計算
function getSnappedVector(dx: number, dy: number): { cos: number; sin: number } {
 ...
 * @param ctx 描画先コンテキスト
 * @param stroke ストロークオブジェクト
 * @param tempCanvas 蛍光ペン用の一時オフスクリーンキャンバス
 */
...
export function usePaintCanvas(): UsePaintCanvasReturn {
  ...
  // メディア状態
  const [mediaType, setMediaType] = useState<PaintMediaType>(null); // 'image' | 'pdf' | null
  const [fileName, setFileName] = useState<string>('');
  const [totalPages, setTotalPages] = useState<number>(0);
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [loadingText, setLoadingText] = useState<string>('');
  const [canvasDimensions, setCanvasDimensions] = useState<Size>({ width: 800, height: 600 });
  ...
  // ページごとの背景データ (キャッシュ)
  const baseImagesRef = useRef<Record<number, { dataUrl: string; width: number; height: number }>>({});

  // ページごとの描画履歴スタック
  const pageHistoriesRef = useRef<Record<number, PageHistory>>({});
  ...
  const [tolerance, setTolerance] = useState<number>(PAINT_CONFIG.DEFAULT_TOLERANCE);
  const [fillOpacity, setFillOpacity] = useState<number>(PAINT_CONFIG.DEFAULT_FILL_OPACITY);
  const [gapClosing, setGapClosing] = useState<number>(PAINT_CONFIG.DEFAULT_GAP_CLOSING);
  ...
  const isShiftSnappedRef = useRef<boolean>(false);
  const shiftLockAxisRef = useRef<{ cos: number; sin: number } | null>(null); // Shift直線用ロック軸 ('horizontal' | 'vertical' | null)
  ...
  // Undo/Redo ボタンの状態を更新
  const updateUndoRedoState = useCallback((pageNum: number) => {
  ...
  // 履歴スタックへのプッシュ操作を一元化
  const pushHistoryEntry = useCallback((pageNum: number, dataUrl: string, strokes: StrokeItem[]) => {
  ...
  // 描画Canvasに画像を復元
  const restorePaintCanvas = useCallback((entry: PageHistoryEntry | string | null | undefined, width: number, height: number) => {
    ...
    const dataUrl = typeof entry === 'object' && entry !== null ? entry.dataUrl : (typeof entry === 'string' ? entry : null);
    if (!dataUrl) return;
  ...
  const renderAllStrokes = useCallback((pageNum: number) => {
  ...
  // ストローク消しゴムのヒット判定と削除
  const checkStrokeErase = useCallback((point: Point) => {
  ...
  const renderPage = useCallback(async (pageNum: number) => {
    ...
    try {
      ...
      if (baseData) {
        ...
        const history = pageHistoriesRef.current[pageNum];
        const latestEntry = history && history.undoStack.length > 0
  ...
  // ページ切り替え
  const changePage = useCallback(async (newPageNum: number) => {
  ...
  // Canvas と履歴の共通初期化
  const setupCanvasData = useCallback((baseData: { dataUrl: string; width: number; height: number }, name: string, numPages: number, isPdf = false) => {
  ...
  const loadFile = useCallback(async (file: File) => {
    ...
    try {
      if (file.type === 'application/pdf') {
        ...
        toast.error('対応していないファイル形式です。画像（JPEG/PNG等）またはPDFを選択してください。');
      }
    } catch (err) {
  ...
  }, [setupCanvasData, toast]);

  // ギャラリーからの画像読み込み
  const loadImageFromDataUrl = useCallback((dataUrl: string, name = 'gallery_image.jpg') => {
    setIsLoading(true);
  ...
  const getMergedCanvasForPage = useCallback(async (pageNum: number): Promise<HTMLCanvasElement | null> => {
    let baseData = baseImagesRef.current[pageNum];
    if (!baseData && pdfDocRef.current) {
      baseData = await renderPdfPage(pdfDocRef.current, pageNum, 2.0);
      baseImagesRef.current[pageNum] = baseData;
    ...
    if (pageNum === currentPage && paintCanvasRef.current) {
      ...
      if (history && history.undoStack.length > 0) {
        drawingDataUrl = history.undoStack[history.undoStack.length - 1]?.dataUrl || null;
      }
    }
  ...
  const getCurrentMergedDataUrl = useCallback(async (): Promise<string | null> => {
  ...
  }, [mediaType, fileName, totalPages, currentPage, getMergedCanvasForPage, toast]);
  ...
  }, [mediaType, fileName, totalPages, saveCurrentPageState, getMergedCanvasForPage, toast]);
