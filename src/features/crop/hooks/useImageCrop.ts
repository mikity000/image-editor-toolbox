...
import { Canvas, FabricImage, Rect, Circle, Ellipse, Polygon, Point, Path, FabricObject } from 'fabric';
import { BoundingBox } from '../../../shared/types/common';
  ...
  const crop = useCallback(
    async (overrideObj: FabricObject | null = null) => {
      ...
      if (
        ...
        tempCanvas.setDimensions({ width: originalImageWidth, height: originalImageHeight });
      }
      tempCanvas.clear();
      ...
      try {
        ...
        // プレビュー用にPNG DataURLを直接設定（WebP変換はダウンロード時のみ実行）
