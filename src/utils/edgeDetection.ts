...
let edgeCanvasContext: CanvasRenderingContext2D | null = null;
 ...
 */
export const initEdgeDetectionCanvas = (fabricCanvas: Canvas | null | undefined): void => {
  ...
  const imgEl: any = (bgImage as any).getElement?.() || (bgImage as any)._element;
  if (!imgEl) return;
 ...
 */
export const clearEdgeDetectionCanvas = (): void => {
 ...
 * @param y Y座標
 * @param radius 探索半径
 * @param threshold エッジ判定の閾値
 * @returns スナップ結果の座標とスナップ有無
 */
export const findClosestEdge = (
  x: number,
  y: number,
  radius: number = CROP_CONFIG.SNAP_RADIUS,
  threshold: number = CROP_CONFIG.MAGNETIC_THRESHOLD_DEFAULT
): EdgeSnapResult => {
