...
export default function PaintComponent() {
  ...
  const [cursorDiameter, setCursorDiameter] = useState(brushSize);
  ...
  // プレビューのスタイル
  const cursorPreviewStyle = useMemo(() => {
    let bgColor = color;
    let opacity = 0.55;
  ...
  const handleSaveToGallery = useCallback(async () => {
    if (!mediaType) return;
    try {
      const dataUrl = await getCurrentMergedDataUrl('image/jpeg', 0.92);
      if (dataUrl) {
        ...
        addImages({ name: `${newName}.jpg`, dataUrl });
      }
    } catch (err) {
