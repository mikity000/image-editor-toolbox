...
import { Canvas, Line, ActiveSelection, FabricImage, FabricObject } from 'fabric';
import { Size } from '../types/common';

export function useSnappingGuides(
  fabricCanvas: Canvas | null,
  guideThickness: number,
  setSelectedSize?: ((size: Size | null) => void) | null,
  saveState?: (() => void) | null
): void {
  useEffect(() => {
    if (!fabricCanvas) return;

    const snapToPixelPosition = (obj: FabricObject) => {
      obj.set({ left: Math.round(obj.left), top: Math.round(obj.top) });
    };

    const snapToPixelScale = (obj: FabricObject) => {
    ...
    const addGuideLine = (coords: [number, number, number, number]) => {
    ...
    const updateGuides = (obj: FabricObject | null = null) => {
      if (!fabricCanvas) return;
      fabricCanvas
        .getObjects()
        .filter((o) => o.isGuide)
      ...
      const aLeft = rectA.left,
        aTop = rectA.top,
        aRight = aLeft + rectA.width,
        aBottom = aTop + rectA.height;
      const tolerance = 0.5;
      ...
      others.forEach((objB) => {
        ...
        const bLeft = rectB.left,
          bTop = rectB.top,
          bRight = bLeft + rectB.width,
          bBottom = bTop + rectB.height;

        if (Math.abs(aRight - bLeft) <= tolerance)
          addGuideLine([aRight, Math.min(aTop, bTop), aRight, Math.max(aBottom, bBottom)]);
        else if (Math.abs(bRight - aLeft) <= tolerance)
          addGuideLine([bRight, Math.min(aTop, bTop), bRight, Math.max(aBottom, bBottom)]);

        if (Math.abs(aBottom - bTop) <= tolerance)
          addGuideLine([Math.min(aLeft, bLeft), aBottom, Math.max(aRight, bRight), aBottom]);
        else if (Math.abs(bBottom - aTop) <= tolerance)
          addGuideLine([Math.min(aLeft, bLeft), bBottom, Math.max(aRight, bRight), bBottom]);
    ...
    const snapDuringMoving = (obj: FabricObject) => {
      ...
      others.forEach((o) => {
        ...
        const oRight = oLeft + o.getScaledWidth(),
          oBottom = oTop + o.getScaledHeight();
      ...
      for (const sv of candidateXs) {
        if (Math.abs(left - sv) <= SNAP_TOLERANCE) left = sv;
      }
      for (const sv of candidateYs) {
        if (Math.abs(top - sv) <= SNAP_TOLERANCE) top = sv;
      }
      obj.set({ left, top });
    };

    const snapDuringScaling = (obj: FabricObject, e: any) => {
      ...
      const offsetX = obj.left - objBoundingRect.left,
        offsetY = obj.top - objBoundingRect.top;
      const candidateXs: number[] = [],
        candidateYs: number[] = [];
      ...
      if (corner.includes('l')) {
        for (const x of candidateXs) {
          if (Math.abs(objBoundingRect.left - x) <= SNAP_TOLERANCE) {
            ...
            if (newWidth > 0) {
              newScaleX = newWidth / objWidth;
              newAttrs.left = x + offsetX;
            }
            break;
          }
        }
      ...
      if (corner.length === 2) {
        if (snappedY && !snappedX && corner.includes('l'))
          newAttrs.left = objBoundingRect.left + objBoundingRect.width - objWidth * newScaleX! + offsetX;
        if (snappedX && !snappedY && corner.includes('t'))
          newAttrs.top = objBoundingRect.top + objBoundingRect.height - objHeight * newScaleY! + offsetY;
      }
