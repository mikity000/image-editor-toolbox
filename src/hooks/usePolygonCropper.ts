...
export function usePolygonCropper(
  ...
  const startPolygonDrawing = useCallback(
    ...
    [fabricCanvasRef, rebuildTempShapes, updateMagneticPreview, updateActiveVertices]
  );
  ...
  const finishPolygonDrawing = useCallback(() => {
    ...
    if (!canvas || polygonPointsRef.current.length < 3) {
      toast.warning('多角形を描くには最低3つの頂点が必要です。');
      return;
    }
    ...
    updateActiveVertices();
    triggerAutoCrop?.();
  }, [fabricCanvasRef, setDrawingObject, setIsDrawingPolygon, setCroppingMode, triggerAutoCrop, updateActiveVertices, toast]);
  ...
  const deleteActiveVertex = useCallback(
    ...
    [fabricCanvasRef, triggerAutoCrop, updateActiveVertices]
  );
